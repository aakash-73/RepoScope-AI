...
export default function GraphPage() {
  ...
  const openSSE = useCallback((repoId) => {
    ...
    const es = new EventSource(
      `${SSE_BASE}/api/v1/analysis/${repoId}/stream${queryString}`
    );
    sseRef.current = es;
  ...
  const handleSyncComplete = useCallback(async (syncedRepoId, syncResult) => {
    ...
    if (syncResult.added > 0 || syncResult.deleted > 0 || syncResult.modified > 0) {
      try {
        // Reset progress UI so the toast and progress bar activate for the new analysis
        setAnalysisStatus(null);
        setCurrentFile(null);
  ...
  // ── Poll semantic graph during analysis to build up incrementally ──────
  useEffect(() => {
    // Only poll in semantic view while analysis is running
    if (viewType !== "semantic" || !selectedRepo || !isAnalyzing) return;

    const interval = setInterval(async () => {
      try {
        const data = await fetchGraph(selectedRepo.repo_id, "semantic");
        if (data) {
          setGraphData(data);
        }
      } catch (err) {
        // Silent — don't disrupt UX on polling failures
      }
    }, 4000);

    return () => clearInterval(interval);
  }, [viewType, selectedRepo?.repo_id, isAnalyzing]);
